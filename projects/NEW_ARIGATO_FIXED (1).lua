...
local GYRO_RINGS = {}
local GYRO_COUNT = 6
local GYRO_RADII = {18, 22, 26, 30, 34, 38}
local GYRO_AXES  = {
...
for i = 1, GYRO_COUNT do
    ...
    local gsel = Instance.new("SelectionBox", gyroFolder)
    gsel.Adornee = gr
    gsel.Color3 = gr.Color
    gsel.LineThickness = 0.04
    gsel.SurfaceTransparency = 1
    local gpl = Instance.new("PointLight", gr)
    gpl.Brightness = 1.8; gpl.Range = 20; gpl.Color = gr.Color
    table.insert(GYRO_RINGS, {part=gr, sel=gsel, pl=gpl, axis=GYRO_AXES[i], radius=r, speed=(i%2==0 and 1 or -1)*(0.28+i*0.07), phase=(i-1)*math.pi/GYRO_COUNT})
end
