...
local lyricStroke, colorCorrection, sound, audioAnalyzer
local lyricOuter, lyricLabel, subLabel, flashFrame
local colorCorrectionR, colorCorrectionB, bloom, blur
local singerHRP, singerHum, singerHead, singerBasePos
local playerCloneHRP, playerCloneHum, playerClone
local playerHRP, playerHum, playerChar, playerHead
local singer, singerParticles, playerParticles
local spotLight, pointLight, singerSpotDown
local stagePlatform, mainFolder, singerNameTag
local zoomBlurFrame, colorSplitFrame1, colorSplitFrame2, scanlines
local conn, singerFlyConn
local karaokeConn
...
playerChar = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
playerHRP  = playerChar:FindFirstChild("HumanoidRootPart")
playerHum  = playerChar:FindFirstChildOfClass("Humanoid")
playerHead = playerChar:FindFirstChild("Head")
...
sky.SkyboxBk = "rbxassetid://159454282"; sky.SkyboxDn = "rbxassetid://159454282"
sky.SkyboxFt = "rbxassetid://159454282"; sky.SkyboxLf = "rbxassetid://159454282"
sky.SkyboxRt = "rbxassetid://159454282"; sky.SkyboxUp = "rbxassetid://159454282"
sky.StarCount = 5000; sky.CelestialBodiesShown = false

colorCorrection = Instance.new("ColorCorrectionEffect", Lighting)
colorCorrection.Brightness = 0.04; colorCorrection.Contrast = 0.25
colorCorrection.Saturation = 0.6;  colorCorrection.TintColor = Color3.fromRGB(200, 120, 255)

colorCorrectionR = Instance.new("ColorCorrectionEffect", Lighting)
colorCorrectionR.TintColor = Color3.fromRGB(255, 160, 160); colorCorrectionR.Enabled = false

colorCorrectionB = Instance.new("ColorCorrectionEffect", Lighting)
colorCorrectionB.TintColor = Color3.fromRGB(160, 160, 255); colorCorrectionB.Enabled = false

bloom = Instance.new("BloomEffect", Lighting)
bloom.Intensity = 1.8; bloom.Size = 32; bloom.Threshold = 0.85

local sunRays = Instance.new("SunRaysEffect", Lighting)
sunRays.Intensity = 0.35; sunRays.Spread = 0.7
...
depthOfField.FarIntensity = 0.1; depthOfField.NearIntensity = 0
depthOfField.FocusDistance = 12; depthOfField.InFocusRadius = 30
...
singer = Players:CreateHumanoidModelFromUserId(Players:GetUserIdFromNameAsync("rhyan571"))
singer.Name = LocalPlayer.Name; singer.Parent = workspace

singerHum  = singer:FindFirstChildOfClass("Humanoid")
singerHRP  = singer:FindFirstChild("HumanoidRootPart")
singerHead = singer:FindFirstChild("Head")
...
playerClone.Name = LocalPlayer.Name; playerClone.Parent = workspace
playerCloneHum = playerClone:FindFirstChildOfClass("Humanoid")
playerCloneHRP = playerClone:FindFirstChild("HumanoidRootPart")
...
local function growPlayerGiant()
    ...
    task.spawn(function()
        ...
        local ringColors = {Color3.fromRGB(255,80,220),Color3.fromRGB(80,200,255),Color3.fromRGB(255,255,80),Color3.fromRGB(80,255,160)}
        for i = 1, 15 do
            ...
            pointGlow.Brightness = 8; pointGlow.Range = 30 + i * 4; pointGlow.Color = ringColors[(i % #ringColors) + 1]
        ...
        task.spawn(function()
            ...
            bigRing.Material = Enum.Material.Neon; bigRing.Color = Color3.fromRGB(255,200,255)
...
spotLight = Instance.new("SpotLight", singerHRP or workspace)
...
pointLight = Instance.new("PointLight", singerHead or workspace)
pointLight.Brightness = 4; pointLight.Range = 35; pointLight.Color = Color3.fromRGB(200, 80, 255)

singerSpotDown = Instance.new("SpotLight", singerHRP or workspace)
...
if singerHRP then
    ...
    stagePlatform.Material = Enum.Material.Neon; stagePlatform.Color = Color3.fromRGB(60, 0, 140)
    ...
    stageRing.Material = Enum.Material.Neon; stageRing.Color = Color3.fromRGB(255, 80, 220)
...
for i = 1, NUM_SHOW_LIGHTS do
    ...
    base.Material = Enum.Material.Neon; base.Color = Color3.fromHSV((i-1)/NUM_SHOW_LIGHTS, 1, 1); base.Transparency = 0.2
    ...
    beam.Width0 = 3; beam.Width1 = 0.05; beam.FaceCamera = true; beam.LightEmission = 1; beam.LightInfluence = 0
    ...
    beam.Transparency = NumberSequence.new({
        NumberSequenceKeypoint.new(0, 0.05), NumberSequenceKeypoint.new(0.7, 0.3), NumberSequenceKeypoint.new(1, 1),
    })
    beam.Segments = 8
...
for i = 1, NUM_RING_ORBIT do
    ...
    ring.Color = Color3.fromHSV((i-1)/NUM_RING_ORBIT, 1, 1); ring.Transparency = 0.3
...
local function updateSpectrum3D(t)
    ...
    local spec; pcall(function() spec = audioAnalyzer:GetSpectrum() end)
...
local function spawnBeatRing(pos, color, bStr)
    task.spawn(function()
        ...
        ring3.Material = Enum.Material.Neon; ring3.Color = color or Color3.fromRGB(255,100,255)
...
local function doColorSplit(duration, strength)
    if G.colorSplitActive or not colorSplitFrame1 then return end
    G.colorSplitActive = true; strength = strength or 0.015
    ...
    task.delay(duration or 0.2, function()
        ...
        task.delay(0.25, function() G.colorSplitActive = false end)
    end)
end
...
for i = 1, GYRO_COUNT do
    ...
    gr.Color = Color3.fromHSV((i-1)/GYRO_COUNT, 1, 1); gr.Transparency = 0.35
    local gpl = Instance.new("PointLight", gr); gpl.Brightness = 1.8; gpl.Range = 20; gpl.Color = gr.Color
...
local function spawnExplosiveCubes(count, origin)
    ...
    for i = 1, count do
        task.spawn(function()
            ...
            cube.Color = Color3.fromHSV(math.random(), 1, 1); cube.CFrame = CFrame.new(origin)
...
local function shockwave(color, scale)
    ...
    task.spawn(function()
        for i = 1, 3 do
            ...
            ring.Color = color or Color3.fromRGB(255,100,255); ring.Anchored = true; ring.CanCollide = false; ring.CastShadow = false
...
singerParticles = addParticles(singerHRP)
playerParticles = addParticles(playerHRP)
...
local EMOTE_IDS = {
    dance  = "rbxassetid://507771019", wave   = "rbxassetid://507770718",
    point  = "rbxassetid://507770453", cheer  = "rbxassetid://507770677",
    laugh  = "rbxassetid://507770818", robot  = "rbxassetid://3360695866",
    flip   = "rbxassetid://507770239", shrug  = "rbxassetid://3360692915",
    spin   = "rbxassetid://507770239", salute = "rbxassetid://3360689775",
}
...
local function playSingerAnim(animId)
    ...
    if singerFlyConn then singerFlyConn:Disconnect(); singerFlyConn = nil; G.singerFlyActive = false end
...
local function singerLevitate(duration)
    ...
    singerFlyConn = RunService.RenderStepped:Connect(function(dt)
        if not singerHRP or not singerHRP.Parent or G.finished then
            singerFlyConn:Disconnect(); singerFlyConn = nil; G.singerFlyActive = false; return
        end
        ...
        if flyT >= (duration or 999) then
            singerFlyConn:Disconnect(); singerFlyConn = nil; G.singerFlyActive = false
...
sg.ResetOnSpawn = false; sg.DisplayOrder = 999; sg.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
lyricLabel.TextXAlignment = Enum.TextXAlignment.Center; lyricLabel.Text = ""; lyricLabel.ZIndex = 11; lyricLabel.RichText = false

subLabel = Instance.new("TextLabel", lyricOuter)
...
subLabel.TextStrokeTransparency = 0.4; subLabel.Font = Enum.Font.Gotham; subLabel.TextSize = 16
subLabel.TextXAlignment = Enum.TextXAlignment.Center; subLabel.Text = ""; subLabel.ZIndex = 11; subLabel.RichText = true

flashFrame = makeFrame(sg, {Size = UDim2.new(1,0,1,0), BackgroundColor3 = Color3.new(1,1,1), BackgroundTransparency = 1, ZIndex = 20})
...
zoomBlurFrame.Name = "zoomblur"; zoomBlurFrame.Size = UDim2.new(1,0,1,0)
zoomBlurFrame.BackgroundTransparency = 1; zoomBlurFrame.ZIndex = 17; zoomBlurFrame.BackgroundColor3 = Color3.new(0,0,0)

colorSplitFrame1 = Instance.new("Frame", sg)
colorSplitFrame1.Size = UDim2.new(1,0,1,0); colorSplitFrame1.BackgroundTransparency = 1
colorSplitFrame1.ZIndex = 21; colorSplitFrame1.BackgroundColor3 = Color3.fromRGB(255,0,100)

colorSplitFrame2 = Instance.new("Frame", sg)
colorSplitFrame2.Size = UDim2.new(1,0,1,0); colorSplitFrame2.BackgroundTransparency = 1
colorSplitFrame2.ZIndex = 21; colorSplitFrame2.BackgroundColor3 = Color3.fromRGB(0,100,255)

scanlines = Instance.new("Frame", sg)
scanlines.Name = "scanlines"; scanlines.Size = UDim2.new(1,0,1,0)
scanlines.BackgroundTransparency = 1; scanlines.ZIndex = 19
...
local function skyColor(r, g, b)
    TweenService:Create(Lighting, TweenInfo.new(1.4, Enum.EasingStyle.Sine), {
        ...
        FogColor = Color3.fromRGB(r//2, g//2, b//2), FogEnd = 600 + math.random(0,300), Brightness = 1.6 + math.random()*0.7,
    }):Play()
    TweenService:Create(bloom, TweenInfo.new(0.6, Enum.EasingStyle.Sine), {Intensity = 1.0 + math.random()*1.5, Size = 22 + math.random()*22}):Play()
end
...
local function floatSinger(height, duration)
    ...
    task.delay(duration, function()
        ...
        if singerHRP and singerHRP.Parent and singerBasePos then
            ...
            task.delay(0.1, function() shockwave(Color3.fromRGB(255,200,80)); burstParticles(singerParticles, 16) end)
        end
    end)
end
...
local function slamDown()
    ...
    task.delay(0.15, function()
        ...
        task.delay(0.25, function()
            ...
            glitch(9, 2.2); shockwave(Color3.fromRGB(255,200,80))
            ...
            doChromaticAberration(0.3, 0.7); doZoomBlurHit(0.18); doColorSplit(0.25, 0.022)
        end)
    end)
end
...
local function doSuperBurst()
    ...
    spawnExplosiveCubes(25, pos); shockwave(Color3.fromRGB(255,60,200))
    ...
    spawnConfetti(35, pos); scaleSinger(5.5, 0.18); task.delay(0.5, function() scaleSinger(3, 0.4) end)
    spawnStarburstRing(); doChromaticAberration(0.45, 0.9)
    addCameraShake(0.6, 0.75); doZoomBlurHit(0.25); doColorSplit(0.35, 0.03)
end
...
local function pulseLyricBeat(bpm)
    ...
    G.lyricEffectConn = RunService.RenderStepped:Connect(function()
        if tick() - lastBeat >= interval then
            ...
            lyricStroke.Color = Color3.fromHSV((G.lyricBeatCount * 0.07) % 1, 1, 1)
...
local function spawnWorldLyric(text)
    ...
    task.spawn(function()
        ...
        sg2.Face = Enum.NormalId.Front; sg2.AlwaysOnTop = true; sg2.SizingMode = Enum.SurfaceGuiSizingMode.PixelsPerStud; sg2.PixelsPerStud = 50
...
local function showLyric(entry)
    ...
    subLabel.TextColor3 = Color3.fromRGB(215, 175, 255); subLabel.TextSize = 20; subLabel.Text = entry.en
    TweenService:Create(subLabel, TweenInfo.new(0.18, Enum.EasingStyle.Quint), {TextSize = 16, TextColor3 = Color3.fromRGB(215, 175, 255)}):Play()
    ...
    TweenService:Create(lyricStroke, TweenInfo.new(0.05), {Color = Color3.fromHSV(hue, 1, 1), Thickness = 4}):Play()
    task.delay(0.45, function() TweenService:Create(lyricStroke, TweenInfo.new(0.35), {Color = Color3.fromRGB(210, 80, 255), Thickness = 2.8}):Play() end)
    ...
    local idx = 0
    karaokeConn = RunService.RenderStepped:Connect(function()
        if G.finished then karaokeConn:Disconnect(); karaokeConn = nil; return end
        if idx < #words then idx = idx + 1; lyricLabel.Text = table.concat(words, " ", 1, idx)
        else karaokeConn:Disconnect(); karaokeConn = nil end
    end)
end
...
local function checkIdleAnim(elapsed)
    if elapsed - G.lastLyricTime > 4 and not G.idleAnimPlaying then
        ...
        if rng <= 3 then
            floatSinger(16, 8); setBarColor(BAR_COLORS.levitate); shockwave(Color3.fromRGB(100,200,255)); burstParticles(singerParticles, 18)
        elseif rng == 4 then
            singerDance(); setBarColor(BAR_COLORS.dance); spawnExplosiveCubes(8, singerHRP and singerHRP.Position)
        elseif rng == 5 then
            singerPoint(); setBarColor(BAR_COLORS.point); zoomToFace("player", 40, 0.4, 2.5)
        elseif rng == 6 then
            singerLaugh(); setBarColor(BAR_COLORS.laugh); glitch(6, 1.2)
        elseif rng == 7 then
            singerWave(); setBarColor(BAR_COLORS.wave); shockwave(Color3.fromRGB(255,200,80))
        elseif rng == 8 then
            singerRobot(); setBarColor(BAR_COLORS.robot); spawnLaserRing(3); task.delay(0.18, function() spawnLaserRing(8) end)
        elseif rng == 9 then
            singerShrug(); setBarColor(BAR_COLORS.shrug); spawnConfetti(18, singerHRP and singerHRP.Position)
        else
            singerSpin(); setBarColor(BAR_COLORS.spin); scaleSinger(4.5, 0.2); task.delay(0.5, function() scaleSinger(3, 0.3) end)
...
local choreo = {
    {0, function() singerDance(); tweenFOV(65, 1.5); colorShift(200,140,255); G.camMode = "orbit"; G.camDist = 22 end},
    {9.22, function()
    ...
    {24.16, function()
        ...
        punchCam(1.3); shockwave(Color3.fromRGB(255,255,80))
    ...
    {40.0, function()
        zoomToFace("player", 36, 0.5, 2.5); singerLaugh(); growPlayerGiant()
    end},
    {45.0, function()
        G.camMode = "face"; G.camDist = 6; tweenFOV(55, 0.4); colorShift(255,180,100)
        spawnExplosiveCubes(12, singerHRP and singerHRP.Position); shockwave(Color3.fromRGB(255,180,100)); singerDance()
    end},
    {50.0, function()
    ...
    {58.45, function()
        ...
        shockwave(Color3.fromRGB(255,255,255)); spawnStarburstRing(); doColorSplit(0.4, 0.025)
    end},
    {1*60+5.73, function()
        zoomToFace("singer", 32, 0.5, 3); colorShift(255,140,200); skyColor(60,0,40); singerCheer()
        spawnPillarRing(12, 18, 16); G.camMode = "dutch"
    end},
    {1*60+14.07, function()
    ...
    {1*60+32.90, function()
        ...
        colorShift(255,80,80); skyColor(80,0,0); doSuperBurst(); singerSalute(); G.camMode = "lowangle"
    end},
    {2*60+11.68, function()
        G.camMode = "orbit"; G.camDist = 18; tweenFOV(68, 0.4)
        colorShift(255,100,255); skyColor(60,0,60); singerDance(); spawnLaserRing(5); spawnPillarRing(8, 12, 12)
    end},
    {2*60+20.0, function()
        zoomToFace("player", 34, 0.5, 2.5); burstParticles(playerParticles, 30); growPlayerGiant()
    end},
    {2*60+29.62, function()
    ...
    {2*60+42.70, function()
        ...
        singerRobot(); spawnSpiralRings(20); G.camMode = "shoulderL"; doColorSplit(0.2, 0.018)
    end},
    {2*60+48.36, function()
        ...
        spawnStarburstRing(); doChromaticAberration(0.5, 1.0); addCameraShake(1.0, 0.7)
    end},
}

sound = PlayGitSound(AUDIO_URL, "ArigatoTokyo", 2, Camera)
audioAnalyzer = Instance.new("AudioAnalyzer", workspace)
...
pcall(function()
    ...
    wire.SourceInstance = audioPlayer; wire.TargetInstance = audioAnalyzer
end)

task.spawn(function()
    for _, s in ipairs(workspace:GetDescendants()) do
        if s:IsA("Sound") and s ~= sound and s.Volume > 0 then G.mutedSounds[s] = s.Volume; s.Volume = 0 end
    end
    workspace.DescendantAdded:Connect(function(d)
        if G.finished then return end
        if d:IsA("Sound") and d ~= sound then task.wait(0.1); G.mutedSounds[d] = d.Volume; d.Volume = 0 end
    end)
end)
...
conn = RunService.RenderStepped:Connect(function(dt)
    ...
    G.elapsed = (sound and sound.Parent and pcall(function() return sound.IsPlaying end) and sound.IsPlaying)
    ...
    if isRealBeat(bass) then
        ...
        if G.beatCount % 4 == 0 then punchCam(0.3 + G.beatStrength * 0.4); doColorSplit(0.12, 0.006 + G.beatStrength * 0.01) end
    ...
    if treble > 0.7 and bass < 0.2 then doColorSplit(0.08, 0.005) end
    if mid > 0.65 then
    ...
    if stagePlatform and singerHRP then
        ...
        stagePlatform.Transparency = 0.22 + math.abs(math.sin(G.elapsed * 2.8)) * 0.18
    end
    ...
    if sound and sound.Parent then pcall(function() sound.RollOffMaxDistance = 9999 end) end
    ...
    if sound and sound.Parent then
        pcall(function() soundDone = not sound.IsPlaying and G.elapsed > 5 end)
    else
    ...
    if soundDone then
        ...
        task.wait(2)
        if G.lyricEffectConn then G.lyricEffectConn:Disconnect() end
        for _, obj in ipairs({sg, mainFolder, singer, playerClone, sky, bloom, colorCorrection, colorCorrectionR, colorCorrectionB, depthOfField, sunRays, blur}) do
